...
// Function to handle double click: Cycle through digits and flash the date
...
void flash_date() {
  ...
  for (int part = 0; part < 2; part++) { // Loop through month digits
    ...
    delay(350); // Show each digit for 0.5 seconds
  }
  
  blank_nixie();
  delay(500); // Add a 1 second space between month and day

  for (int part = 2; part < 4; part++) { // Loop through day digits
    blank_nixie(); // Blank tube before showing next digit
    display_digit(dateStr[part]); // Display current part of the day
    delay(350); // Show each digit for 0.4 seconds
  }
